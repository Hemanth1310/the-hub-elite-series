...
export default function RoundsV1() {
  ...
  const [roundDetails, setRoundDetails] = useState<any | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      ...
      const { data: predictions } = await supabase
        ...
        .select('*')
        .eq('round_id', round.id);
      setRoundDetails({ round, matches, predictions });
      setLoading(false);
    };
    fetchDetails();
  }, [selectedRound]);
  ...
  if (selectedRound && roundDetails) {
    const { round, matches, predictions } = roundDetails;
    const myPrediction = predictions.find((p: any) => p.user_id === user?.id);
    // Calculate my rank
    const sortedPredictions = [...predictions].sort((a, b) => b.points - a.points);
    const myRank = sortedPredictions.findIndex((p: any) => p.user_id === user?.id) + 1;
    // Winner and average score (fallback to round fields)
    const winner = round.winner || (sortedPredictions[0] ? sortedPredictions[0].user_id : '');
    const averageScore = round.average_score || (predictions.length ? (predictions.reduce((sum: number, p: any) => sum + (p.points || 0), 0) / predictions.length).toFixed(1) : 0);
      ...
      <LayoutV1>
        <div className="mb-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-2">
            ...
            <Link href={`/version1/rounds/${round.number}/compare`}>
        ...
        <Card className="bg-slate-900 border-slate-800 p-4 sm:p-6 mb-8">
          ...
          <div className="sm:hidden space-y-3">
            {matches.map((match: any, idx: number) => {
              const myPick = myPrediction?.picks ? myPrediction.picks[idx] : null;
              const isConviction = myPrediction?.conviction === idx;
              const isCorrect = myPick === match.result;
              return (
                <div key={match.id} className={`bg-slate-800/50 rounded-lg p-4 space-y-2 ${match.is_match_of_the_week ? 'ring-2 ring-yellow-500/50' : ''}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      <span className="text-white text-sm font-medium">
                        <span className="sm:hidden">{match.away_team?.short_name}</span>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-semibold ${isCorrect ? 'text-green-400' : 'text-red-400'}`}>{myPick}</span>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Card className="bg-slate-900 border-slate-800 p-6">
            ...
            <div className="text-sm text-slate-500 mt-1">of {predictions.length} players</div>
          </Card>
          <Card className="bg-slate-900 border-slate-800 p-6">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-blue-500/10 rounded-lg flex items-center justify-center flex-shrink-0">
                  <span className="text-blue-400 font-bold text-lg">{round.number}</span>
                </div>
                <div className="flex-1">
                  <div className="text-white font-semibold mb-1">Round {round.number}</div>
              ...
              <div className="flex items-center justify-between gap-4 pl-0 sm:pl-14">
                ...
                <div className="flex-1 text-center">
                  ...
                  <div className="text-white font-bold">{round.average_score ? round.average_score.toFixed(1) : '-'}</div>
                </div>
